...
int SPRITE::Action(int action,int var1,int var2,int var3)
{
    ...
    case 134:
        if (Map->ValidateVid(var1)) {
            const int query=var1<MAP::kRetailVidCapacity ? var1+0x800 : MAP::EncodeVidQuery(var1);
            SPRITE* victim=Map->GetSpriteScr(query,static_cast<float>(var2),static_cast<float>(var3));
            if (victim)
