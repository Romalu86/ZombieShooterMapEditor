    ...
    : MAP(instance,prev,command_line,sw,init), selectedSprites(), undo(), editFileName()
{
    ...
    for (int i=0;i<5;++i) {
        ...
        if (nvid<=0 || nvid>=m_noVid || !VidSlot(nvid) || nameDrawingVid[i]=="")
            continue;
        ...
        drawingVid.Read(&VidSlot(nvid)->m_extraTypeFlags,2u);
        drawingVid.Read(&VidSlot(nvid)->m_phaseRandomInterval,2u);
        drawingVid.Read(&VidSlot(nvid)->m_dotFrameCount,2u);
        drawingVid.Read(&VidSlot(nvid)->m_regionTileStepX,2u);
        drawingVid.Read(&VidSlot(nvid)->m_regionTileStepY,2u);
        VidSlot(nvid)->Load(&drawingVid);
    }
    ...
    VID* initialMouseVid=(m_noVid>1 && VidSlot(1)) ? VidSlot(1) : EmptyVid;
