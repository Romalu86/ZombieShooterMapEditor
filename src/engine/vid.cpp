...
void VID::SetChildAndLink()
{
    if (m_linkVidIndex) {
        ...
        if (linkIndex>=0 && linkIndex<Map->m_noVid && Map->m_vids[linkIndex])
            m_linkVid=Map->m_vids[linkIndex]->m_exchangeVid;
        else
    ...
    while (chainedLinkIndex>=0 &&
           ...
           Map->m_vids[chainedLinkIndex] &&
           chainedLinkIndex!=0) {
        chainedLinkIndex=Map->m_vids[chainedLinkIndex]->m_linkVidIndex;
    }
    ...
    for (i=0;i<17;++i) {
        ...
        const int childIndex=child<0
            ...
            : child;
        if (childIndex>=0 && childIndex<Map->m_noVid && Map->m_vids[childIndex]) {
            VID* mirror=Map->m_vids[childIndex]->m_exchangeVid;
