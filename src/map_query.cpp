...
{
    if (IsExtendedVidQuery(type))
        return true;
    const int vidIndex=type&0x0FFF;
    return vidIndex!=0 && (type&0x1000)==0;
}

int VidQueryIndex(int type)
{
    return IsExtendedVidQuery(type) ? (type&0x1FFF) : (type&0x0FFF);
}

bool IsSpriteClassQuery(int type)
{
    return !IsExtendedVidQuery(type) && (type&0x1000)!=0;
}
...
SPRITE* MAP::GetSpriteScr(int type,float screenX,float screenY)
{
    ...
    int spriteType;
    int vidIndex=VidQueryIndex(type);
    ...
    if (vidQuery) {
        ...
        VID* queryVid=ValidateVid(vidIndex) ? VidSlot(vidIndex) : EmptyVid;
        if (queryVid->m_entitiesNumber[0]+queryVid->m_entitiesNumber[1]+
            queryVid->m_entitiesNumber[2]+queryVid->m_entitiesNumber[3]==0)
            return 0;

        queryVid=ValidateVid(vidIndex) ? VidSlot(vidIndex) : EmptyVid;
        if ((queryVid->m_flag&0x40u)!=0)
            type|=0x8000;

        vidIndex=VidQueryIndex(type);
        queryVid=ValidateVid(vidIndex) ? VidSlot(vidIndex) : EmptyVid;
    ...
    if (type&0x8000) {
        for (SPRITE* sprite=Hash->FirstInBox(screenX-radius,
            ...
            if (sprite->m_parent ||
                ...
                (IsSpriteClassQuery(type) && spriteVid->m_spriteClass!=static_cast<unsigned int>(type&0x7FF)) ||
                ((VidQueryIndex(type)) && spriteVid->m_idx!=(VidQueryIndex(type))))
                continue;
    ...
    if (vidQuery) {
        ...
        VID* queryVid=(vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex))
            ? VidSlot(vidIndex) : EmptyVid;
    ...
    VID* beginVid=(vidQuery && vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex))
        ? VidSlot(vidIndex) : EmptyVid;
    const int beginLayer=vidQuery ? beginVid->m_layer : 0;
    VID* endVid=(vidQuery && vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex))
        ? VidSlot(vidIndex) : EmptyVid;
...
SPRITE* MAP::FindNearestSprite(int type,float x,float y,float radius,SPRITE* prev)
{
    ...
    int spriteType;
    if (vidQuery) {
        VID* queryVid=(vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex)) ? VidSlot(vidIndex) : EmptyVid;
        ...
        queryVid=(vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex)) ? VidSlot(vidIndex) : EmptyVid;
    ...
    if (vidQuery) {
        ...
        if (queryVid->m_spriteClass==10u ||
            ((queryVid=(vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex)) ? VidSlot(vidIndex) : EmptyVid),queryVid->m_spriteClass==19u)) {
            int index;
            SPRITE* sprite=0;
            if (m_menu.m_no) {
                index=m_menu.m_no-1;
...
void MAP::FindSpritesInsidePolygon(int type,const POLYGON* polygon,SPRITE_LIST* list)
{
    ...
    int endLayer=20;
    if (vidQuery) {
        VID* beginVid=(vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex)) ? VidSlot(vidIndex) : EmptyVid;
        beginLayer=beginVid->m_layer;
        VID* endVid=(vidIndex>=0 && vidIndex<m_noVid && VidSlot(vidIndex)) ? VidSlot(vidIndex) : EmptyVid;
