...
void MAP_EDIT::DeleteUnit(float x,float y,int sprite_type)
{
    SPRITE* sprite;
    if (optAirBrush) {
        const int nvid=Mouse->Vid()->m_idx;
        const int query=nvid<MAP::kRetailVidCapacity ? nvid+0x800 : MAP::EncodeVidQuery(nvid);
        sprite=FindNearestSprite(query,x,y,static_cast<float>(optAirBrushSize),0);
    } else {
